...
class Pawn extends GameObject_1.default {
    constructor(x, y, width, height, color = "red", health = 10) {
        super(Vector_1.vector(x, y), width, height);
        this.offset = Vector_1.vector(0, 0);
        this.color = color;
        this.health = health;
        this.sprite = null;
    }
    updateToFixed() {
        if (this.offset.length > 0) {
            ...
            if (this.position.subtract(this.offset).length > 2) {
                let fixVector = Vector_1.vector(0, 0).subtract(this.offset);
    ...
    drawBasic() {
        Canvas_1.default.vrect(this.position.add(this.offset), this.dimensions).fillStyle(this.color).fill();
        if (this.health <= 0)
    ...
    drawHealthBar() {
        ...
        Canvas_1.default.fillStyle("black")
            .text(this.health.toString(), healthNumPos.x, healthNumPos.y, 25, "25px pixel");
    }
}
